...
const PostItem: React.FC<PostItemProps> = ({ data, userId }) => {
    ...
    const pushToUser = useCallback((event: any) => {
        // this'll override the parent onclick elements
            ...
            <div className="flex flex-row items-start gap-3">
                ...
                <div>
                    <div className="flex flex-row items-center gap-2">
                        <p onClick={pushToUser} className="text-white font-semibold cursor-pointer hover:underline">{data.user.name}</p>
                        <span onClick={pushToUser} className="text-neutral-500 cursor-pointer hover:underline hidden md:block">@{data.user.username}</span>
